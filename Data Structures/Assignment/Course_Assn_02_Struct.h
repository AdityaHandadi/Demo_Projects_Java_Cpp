...
*/ 
	...
	int row_index=0;
	//element * row_array=new element[n/4];
	element row_array[n/4];
	int col_index=0;
	element col_array[n/5];
	//element * col_array=new element[n/5];
	int sum=0;
	...
	for(int i=0;i<n/4;i++){
		...
		//cout<<"Values :"<<row[i]<<"||Index :"<<i<<"||VALUE IN STRUCT :"<<row_array[row_index].val<<endl;
	...
	cout<<"ROW ARAY "<<sizeof(row_array)/sizeof(*row_array)<<endl;
	...
	while(row_in<=row_lim && col_in<=col_lim){
